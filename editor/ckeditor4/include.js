...
Dida.module.editor.ckeditor.uploadInfo = function(iframeId, type, edit) {
  $(iframeId).load(function() {
    ...
    var html = '';
    if (type == 'image' && !edit.config.dida_upload_disabled_size) {
      html += '<div><label>' + Dida.t('editor', '小图尺寸') + '</label><input type="text" size="10" name="thumb"'; 
      if (edit.config.dida_upload_default_size) html += ' value="' + edit.config.dida_upload_default_size + '"';
      html += ' placeholder="' + Dida.t('editor', '200x100') + '"/>';
      html += '<span>' + Dida.t('editor', '宽度x高度，如：200x100。不需生成缩略图请留空') + '</span></div>';
    }
  ...
  CKEDITOR.on('dialogDefinition', function(ev)  {
    ...
    if (dialogName == 'image' || dialogName == 'link') {
      ...
      dialogDefinition.onLoad = function() {
        ...
        Dida.module.editor.ckeditor.uploadInfo('#' + fileField._.frameId, dialogName, ev.editor);
      }
    }
  });
}
